...
function Band({
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      ...
      if (j3.current && fixed.current && j1.current && j2.current && band.current) {
        ...
        geometry.setPoints((curve as any).getPoints(isMobile ? 16 : 32));
      }
      if (card.current) {
    ...
    <>
      <group position={[0, 4, 0]}>
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          {/* Scaled up by 2x uniformly: 0.8 * 2 = 1.6, 1.125 * 2 = 2.25 */}
          <CuboidCollider args={[1.6, 2.25, 0.01]} />
          <group
            scale={4.5} // 2.25 * 2
            position={[0, -2.4, -0.05]} // -1.2 * 2 = -2.4
